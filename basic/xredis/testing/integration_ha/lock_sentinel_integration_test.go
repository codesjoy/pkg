...
func TestSentinelLockLifecycle(t *testing.T) {
	...
	_, err = locker.TryAcquire(ctx, "it:ha:sentinel:lock", 1200*time.Millisecond)
	require.ErrorIs(t, err, lock.ErrNotObtained)

	require.NoError(t, lease.Release(ctx))

	reacquired, err := locker.TryAcquire(ctx, "it:ha:sentinel:lock", 1200*time.Millisecond)
	require.NoError(t, err)
