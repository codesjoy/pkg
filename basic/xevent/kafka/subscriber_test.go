...
func TestNewSubscriberValidate(t *testing.T) {
	...
	_, err = NewSubscriber(SubscriberConfig{Consumer: &xkafka.GroupConsumer{}})
	if !errors.Is(err, ErrNilDispatcher) {
		t.Fatalf("expected ErrNilDispatcher, got %v", err)
	}
}

func TestSubscriberSubscribeDispatchesKafkaMessage(t *testing.T) {
	value := []byte(`{"id":"evt_1"}`)
		...
		msg: &consume.MessageContext{
			Message: &sarama.ConsumerMessage{
				...
				Headers: []*sarama.RecordHeader{
					{Key: []byte(defaultEventTypeHeader), Value: []byte("order.created")},
				},
			},
		},
	}
	...
	if string(value) != `{"id":"evt_1"}` {
		t.Fatalf("payload should be copied before delivery, got %q", string(value))
	}
}
...
func TestSubscriberSubscribeValidationAndErrors(t *testing.T) {
	...
	err = subscriber.Subscribe(context.Background())
	if !errors.Is(err, ErrEventTypeHeaderRequired) {
		t.Fatalf("expected ErrEventTypeHeaderRequired, got %v", err)
	}
